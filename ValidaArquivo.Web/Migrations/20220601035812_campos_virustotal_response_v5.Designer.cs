// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using ValidaArquivo.Data;

#nullable disable

namespace ValidaArquivo.Web.Migrations
{
    [DbContext(typeof(ValidaArquivoContext))]
    [Migration("20220601035812_campos_virustotal_response_v5")]
    partial class campos_virustotal_response_v5
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "6.0.5")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder, 1L, 1);

            modelBuilder.Entity("ValidaArquivo.Domain.Entities.AnaliseArquivo", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Antivirus")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ArquivoUploadId")
                        .HasColumnType("int");

                    b.Property<DateTime?>("DataAlteracao")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime2");

                    b.Property<bool>("Detected")
                        .HasColumnType("bit");

                    b.Property<int?>("IdArquivoUpload")
                        .HasColumnType("int");

                    b.Property<string>("Result")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Update")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Version")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("ArquivoUploadId");

                    b.ToTable("AnaliseArquivos");
                });

            modelBuilder.Entity("ValidaArquivo.Domain.Entities.ArquivoUpload", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<byte[]>("Conteudo")
                        .IsRequired()
                        .HasColumnType("varbinary(max)");

                    b.Property<DateTime?>("DataAlteracao")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DataEnvioVirusTotal")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DataResultadoVirusTotal")
                        .HasColumnType("datetime2");

                    b.Property<string>("ErroEnvioVirusTotal")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ErroRetornoVirusTotal")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NomeArquivo")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("VirusTotalPermalink")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("VirusTotalResource")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("VirusTotalResponseCode")
                        .HasColumnType("int");

                    b.Property<string>("VirusTotalSHA256")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("VirusTotalScanId")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("VirusTotalVerboseMessage")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("ArquivoUploads");
                });

            modelBuilder.Entity("ValidaArquivo.Domain.Entities.AnaliseArquivo", b =>
                {
                    b.HasOne("ValidaArquivo.Domain.Entities.ArquivoUpload", "ArquivoUpload")
                        .WithMany("AnaliseArquivos")
                        .HasForeignKey("ArquivoUploadId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ArquivoUpload");
                });

            modelBuilder.Entity("ValidaArquivo.Domain.Entities.ArquivoUpload", b =>
                {
                    b.Navigation("AnaliseArquivos");
                });
#pragma warning restore 612, 618
        }
    }
}
